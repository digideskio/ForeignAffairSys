<?php   
$filename='教师因公出国（境）申请表-领队-'.$model->leader->name.'.doc';
$filename=iconv("UTF-8", "GBK", "教师因公出国（境）申请表-领队-".$model->leader->name.".doc");
...
foreach($model->visitors as $k=>$v)
      {
        ...
        {  
            ...
            for($empty;$empty>1;$empty--){//  style='height:".$height."mm'
...
<td rowspan=<?php echo count($model->itineraries)?>>本次出<br>访国家<br>(地区)</td>
         <?php 
              //echo count($visit->itineraries);
           //echo count($model->itineraries);
